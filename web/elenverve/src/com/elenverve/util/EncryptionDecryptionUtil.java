...
public class EncryptionDecryptionUtil {
     ...
     public static String decrypt(byte[] encdata) {
             //logger.debug("decrypt(): data length=" + encdata.length);
             // ----- Decrypt the file
             try {
     ...
     public static byte[] encrypt(byte[] data) {
             try
             {
                     ...
                     //logger.debug("Creating a temp file...");
                     // create a file and write the string to it
                     File tempfile = File.createTempFile("pgp", null);
                     FileOutputStream fos = new FileOutputStream(tempfile);
                     fos.write(data);
                     fos.close();
                     //logger.debug("Temp file created at ");
                     //logger.debug(tempfile.getAbsolutePath());
                     //logger.debug("Reading the temp file to make sure that the bits were written\n--------------");
                     ...
                     //logger.debug("Key Strength = " + key.getBitStrength());
                     //logger.debug("Algorithm = " + key.getAlgorithm());
                     //logger.debug("Bit strength = " + key.getBitStrength());
                     //logger.debug("Version = " + key.getVersion());
                     //logger.debug("Encryption key = " + key.isEncryptionKey()+ ", Master key = " + key.isMasterKey());
                     int count = 0;
                     ...
                     //logger.debug("encrypted text length=" + baos.size());			
                     tempfile.delete();
             ...
             catch (PGPException e)
             {
                     ...
                     //logger.debug(e.getUnderlyingException().toString());
                     e.printStackTrace();
             }
             catch (Exception e)
             {
     ...
     private static PGPPublicKey readPublicKeyFromCol(InputStream in)
             ...
             //logger.debug("key ring size=" + pkCol.size());
                         ...
                         throws IOException, NoSuchProviderException, PGPException
     {
             ...
             //logger.debug("creating comData...");
             // get the data from the original file
             PGPCompressedDataGenerator comData = new PGPCompressedDataGenerator(PGPCompressedDataGenerator.ZIP);
             PGPUtil.writeFileToLiteralData(comData.open(bOut), PGPLiteralData.BINARY, new File(fileName));
             comData.close();
             //logger.debug("comData created...");
             //logger.debug("using PGPEncryptedDataGenerator...");
             ...
             //logger.debug("used PGPEncryptedDataGenerator...");
             // take the outputstream of the original file and turn it into a byte
             // array
             byte[] bytes = bOut.toByteArray();
             //logger.debug("wrote bOut to byte array...");
     ...
     private static String _decrypt(InputStream in, InputStream keyIn,
             ...
             try {
                     ...
                     while (sKey == null && it.hasNext()) {
                             ...
                             //logger.debug("pbe id=" + pbe.getKeyID());
     ...
     public static void main(String[] args) {
             ...
             //logger.debug("Encrypted: " + encdata);
             ...
             //logger.debug("byte array" + newB.length);
             // ----- Decrypt the token that
             String result = decrypt(newB);
             //logger.debug("Decrypted: " + result);
     }
}
