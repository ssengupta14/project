	 ...
	 */
	 public List<ProductDvo> getProducts() {
		if (products == null) {
			...
			logger.debug("retrieved [" + products.size()
					+ "] products from db..");
		}else{
			logger.debug("returning [" + products.size()
	 ...
	 public List<CollectionDvo> getCollections(){
		 if(collections==null){
			 ...
			 collections = mongoTemplate.findAll(CollectionDvo.class, "collectionDvo");
				logger.debug("retrieved [" + collections.size()
						+ "] collections from db..");
		 }else{
				logger.debug("returning [" + collections.size()
	 ...
	 public List<CategoryDvo> getCategories(){
		 if(categories==null){
			 ...
			 categories = mongoTemplate.findAll(CategoryDvo.class, "categoryDvo");
				logger.debug("retrieved [" + categories.size()
						+ "] categories from db..");
		 }else{
				logger.debug("returning [" + categories.size()
	 ...
	 public List<OfferDvo> getOffers(){
		 if(offers==null){
			 ...
			 offers = mongoTemplate.findAll(OfferDvo.class, "offerDvo");
				logger.debug("retrieved [" + offers.size()
						+ "] offers from db..");
		 }else{
				logger.debug("returning [" + offers.size()
						+ "] offers from service...");
			}
		 return offers;
	 }
